...
from model.ITSymNet.text_part.CLIP.simple_tokenizer import SimpleTokenizer as _Tokenizer
from model.ITSymNet.text_part.CLIP.CLIP_tokenize import tokenize
from model.ITSymNet.text_part.Transformer import Transformer, LayerNorm
...
class Custom_Text_Prompt(nn.Module):
    def __init__(self, n_vector: int, vector_dim: int, keywords: str, token_embedding):
        ...
        self.register_buffer("token_suffix", embedding[:, n_vector:, :])
    ...
    def forward(self):
        ...
        prompts = torch.cat([text_vector, suffix], dim=1)
